...
The confirmation page fetches authenticated payment data rather than trusting `type=purchase` in the URL. New redirects include the order ID. Older redirects without an order ID use the account's latest stored paid order. Invoice and receipt endpoints require a session belonging to that paid order; unpaid, rejected, and other-account orders return no document.

Use **View & save invoice** on the confirmation page, then the browser Print menu → Save as PDF. The HTML is print-styled, escapes customer data, and disallows scripts. Test payments have a prominent test label. The account portal lists invoices and offers an email attachment on request after email verification. SMTP must be configured; delivery failures are reported without changing the payment result.
