...
- `customers`: profile, bcrypt password hash, hashed reset/verification tokens, session version, signup timestamp, trial request, onboarding status, purchase history.
...
- `npm run test:mongodb` from `backend`: opt-in real Atlas integration checks. Creates a uniquely named `loomiq_verification_*` database and removes only that database afterward. Validates HTTP signup/login, stored password hashes and sessions, trial eligibility, durable state after repository recreation, pricing, concurrent signup, transaction rollback, safe migration, and demo submission storage.
- `npm run build --prefix frontend` and `npm run lint --prefix frontend`: website checks.

Use the same secret configuration on the deployment host; local configuration does not deploy the website. Email verification and password recovery are implemented. ERP provisioning remains manual; operators publish the provisioned workspace link through the onboarding tool. Gateway webhook retries remain necessary for refund failures; no independent refund worker is installed. See deployment.md for backup, recovery, and health monitoring.
