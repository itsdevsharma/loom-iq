...
export default function AccountPage() {
  ...
  return <main className="account-page">
    ...
    {data && <>
      ...
      <section className="account-card account-workspace"><div className="account-card-heading"><div><p className="account-eyebrow">Workspace</p><h2>{cmsValue("AccountPage.15", "Your workspace")}</h2></div><span className="account-workspace-icon" aria-hidden="true">↗</span></div>
        ...
        {data.workspaceUrl ? <a className="button button-primary" href={data.workspaceUrl} rel="noreferrer">{cmsValue("AccountPage.20", "Open workspace")} <span aria-hidden="true">↗</span></a> : <a className="button button-primary" href={base + 'demo'}>{cmsValue("AccountPage.21", "Start 3-hour demo")} <span aria-hidden="true">↗</span></a>}
        <p className="account-workspace-note">{cmsValue("AccountPage.22", "Membership purchases are one-time payments. There are no automatic charges.")}</p>
      </section></div>
      <section className="account-card account-billing"><div className="account-card-heading"><div><p className="account-eyebrow">Billing</p><h2>{cmsValue("AccountPage.27", "Payments & invoices")}</h2></div>{data.invoices.length > 0 && <span className="account-count">{data.invoices.length}</span>}</div>
        {!data.invoices.length ? <div className="account-empty-state"><span aria-hidden="true">₹</span><div><p>{cmsValue("AccountPage.28", "No confirmed payments yet.")}</p><a href={base + 'payment'}>{cmsValue("AccountPage.29", "Review plans")} <span aria-hidden="true">→</span></a></div></div> : <ul className="account-invoices">{data.invoices.map(invoice => <li key={invoice.orderId}>
