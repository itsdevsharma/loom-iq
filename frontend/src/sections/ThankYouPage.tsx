...
export default function ThankYouPage() {
  ...
  return <main className="confirmation-page">
    <header className="confirmation-header"><a href={base} className="confirmation-brand"><img src={cmsValue("ThankYouPage.1", logo)} alt="" />{cmsValue("ThankYouPage.2", "Loom")}<span>{cmsValue("ThankYouPage.3", "IQ")}</span></a><a href={cmsValue("ThankYouPage.4", "#support")}>{cmsValue("ThankYouPage.5", "Need a hand? Contact support â†—")}</a></header>
    <div className="confirmation-layout"><section className={`confirmation-card${receipt ? " confirmation-card--receipt" : ''}`}>
      <div className="confirmation-intro">
        <span className="confirmation-check" aria-hidden="true">{confirmed ? 'âœ“' : 'â€¦'}</span>
        ...
        <h1>{purchase ? receipt ? cmsValue("ThankYouPage.11", "Youâ€™re one step closer to better work.") : cmsValue("ThankYouPage.12", "Letâ€™s confirm your purchase.") : trial ? cmsValue("ThankYouPage.13", "Your next chapter starts here.") : cmsValue("ThankYouPage.14", "Letâ€™s build a clearer workflow.")}</h1>
        <p className="confirmation-lead">{purchase ? receipt ? cmsTemplate("ThankYouPage.extra49", "Thank you{0}. Your LoomIQ {1} payment is confirmed. Weâ€™ll help you get started.", [receipt.customer.name ? ', ' + receipt.customer.name : '',receipt.plan]) : cmsValue("ThankYouPage.15", "Weâ€™re retrieving your payment details and invoice securely.") : trial ? cmsValue("ThankYouPage.16", "Your trial request is with our team. Weâ€™ll get in touch to arrange your access.") : cmsValue("ThankYouPage.17", "Your demo request is received. Our team will contact you to arrange a personalized demo, then help you decide whether trial access or a paid plan fits your needs.")}</p>
        {receipt?.testMode && <p className="confirmation-test">{cmsValue("ThankYouPage.18", "Test Mode Â· No real money was charged.")}</p>}
      </div>
      ...
      {receipt && <section className="confirmation-receipt" aria-label={cmsValue("ThankYouPage.21", "Payment summary")}>
        <div className="confirmation-receipt-heading"><div><span className="confirmation-eyebrow">{cmsValue("ThankYouPage.22", "Your membership")}</span><h2>{cmsValue("ThankYouPage.23", "LoomIQ ")}{receipt.plan}</h2><p>{cmsValue("ThankYouPage.24", "First month Â· One-time payment")}</p></div><div className="confirmation-amount"><small>{receipt.testMode ? cmsValue("ThankYouPage.25", "Test amount") : cmsValue("ThankYouPage.26", "Amount paid")}</small><strong>â‚¹{(receipt.amount / 100).toLocaleString('en-IN')}</strong></div></div>
