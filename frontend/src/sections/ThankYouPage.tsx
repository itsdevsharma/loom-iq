  ...
  return <main className="confirmation-page">
    ...
    <div className="confirmation-layout"><section className={`confirmation-card${receipt ? " confirmation-card--receipt" : ''}`}>
      <div className="confirmation-intro">
        ...
        <h1>{purchase ? receipt ? cmsValue("ThankYouPage.11", "You’re one step closer to better work.") : cmsValue("ThankYouPage.12", "Let’s confirm your purchase.") : trial ? cmsValue("ThankYouPage.13", "Your next chapter starts here.") : cmsValue("ThankYouPage.14", "Let’s build a clearer workflow.")}</h1>
        <p className="confirmation-lead">{purchase ? receipt ? cmsTemplate("ThankYouPage.extra49", "Thank you{0}. Your LoomIQ {1} payment is confirmed. We’ll help you get started.", [receipt.customer.name ? ', ' + receipt.customer.name : '',receipt.plan]) : cmsValue("ThankYouPage.15", "We’re retrieving your payment details and invoice securely.") : trial ? cmsValue("ThankYouPage.16", "Your trial request is with our team. We’ll get in touch to arrange your access.") : cmsValue("ThankYouPage.17", "Your demo request is received. Our team will contact you to arrange a personalized demo, then help you decide whether trial access or a paid plan fits your needs.")}</p>
      ...
      {receipt && <section className="confirmation-receipt" aria-label={cmsValue("ThankYouPage.21", "Payment summary")}>
        ...
        <div className="confirmation-invoice-action"><a className="confirmation-primary" href={`${import.meta.env.VITE_API_URL ?? ''}/api/purchase/invoice/${encodeURIComponent(receipt.orderId)}`} target="_blank" rel="noreferrer">{cmsValue("ThankYouPage.31", "View & save invoice ")}<span aria-hidden="true">↗</span></a><span>{cmsValue("ThankYouPage.32", "Print or save as PDF from your browser.")}</span></div>
      </section>}
