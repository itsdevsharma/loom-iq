  ...
  return <main className="signup-page">
    ...
    {!ready ? <p role="status">{cmsValue("SignupPage.12", "Loading your account…")}</p> : !offer.signedUp ? (
      ...
      <section className="signup-card" id="account">
        ...
        <p className="signup-intro">{login ? cmsValue("SignupPage.37", "Sign in to pick up where you left off. Review your plan and account details.") : "Create your account, then enter billing details securely at checkout."}</p>
        {!login && <div className="signup-no-card"><LockIcon />{cmsValue("SignupPage.39", " No card needed to create an account")}</div>}
        <form onSubmit={submit} onFocus={() => { setFormStartedAt(value => value || Date.now()); if (!signupStarted.current) { signupStarted.current = true; trackEvent('signup_started'); } }}>
          <fieldset disabled={busy}>
            ...
            <button className="button button-primary" type="submit">{busy ? cmsValue("SignupPage.58", "Please wait…") : login ? cmsValue("SignupPage.59", "Sign in") : "Create account & continue →"}</button>
          </fieldset>
        </form>
