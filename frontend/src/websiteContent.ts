...
export async function loadWebsiteContent() {
  try {
    const response = await fetch(`${import.meta.env.VITE_API_URL ?? ''}/api/content/website`, { signal: AbortSignal.timeout(1500) })
...
export function cmsValue<T>(key: string, fallback: T): T {
  ...
  function interpolate(item:unknown):unknown {
    if(typeof item==='string') return item.replace(/\{(discount|starterOffer|starterRegular|growthOffer|growthRegular|enterpriseRegular)\}/g, (_,name)=>replacements[name])
...
export function websitePricing() {
  return cmsValue('Site.pricing', {Starter:{firstMonth:995,recurring:1990},Growth:{firstMonth:1495,recurring:2990},Enterprise:{recurring:4990}})
}
