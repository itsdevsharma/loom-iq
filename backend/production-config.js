function validateProduction(env = process.env) {
  ...
  const erpLogin = new URL(env.ERP_LOGIN_URL);
  if (erpLogin.protocol !== 'https:' || erpLogin.username || erpLogin.password) throw new Error('Production ERP_LOGIN_URL must be an HTTPS URL without embedded credentials.');
  if (env.ERP_MARKETING_INTEGRATION_TOKEN.length < 32) throw new Error('Use an ERP_MARKETING_INTEGRATION_TOKEN of at least 32 characters.');
